...
- [x] **Milestone 3c: Ship logs through Alloy to Grafana Cloud Loki** — Add `loki.source.kubernetes` and `loki.write` components to Alloy River config to tail pod logs and forward to Grafana Cloud Loki. Use the existing `GRAFANA_CLOUD_API_TOKEN` for authentication. Validate using Grafana MCP `query_loki_logs` to find recent pod logs (e.g., `{namespace="dot-ai"}`)
- [x] **Milestone 4: Create dashboards in Grafana Cloud** — Imported community dashboard `kube-state-metrics-v2` (gnetId 13332, rev 12) into "Kubernetes" folder via Grafana API. Adapted datasource UIDs to `grafanacloud-prom`, fixed 2 deprecated metric names for kube-state-metrics v2. Dashboard at `https://dot.grafana.net/d/k8s-kube-state-metrics-v2/kube-state-metrics-v2`. Covers Cluster, Node, Deployments, StatefulSets, Pods, Containers, HPA, Jobs, PVC. Logs and traces use Grafana Cloud Explore natively (no dashboards needed). Validated live data via MCP `query_prometheus` and `search_dashboards`
- [x] **Milestone 5: Update documentation** — Updated `CLAUDE.md`: added Grafana Cloud + Alloy to Key Technologies, `alloy.yaml` to Argo CD Applications, `GRAFANA_SERVICE_ACCOUNT_TOKEN` to required env vars, updated MCP Integration to list both dot-ai and Grafana servers. `.env.vals.yaml` already had `GRAFANA_SERVICE_ACCOUNT_TOKEN`. Verified against live Grafana Cloud state via MCP `list_datasources` and `search_dashboards`
- [x] **Cleanup: Revert Argo CD targetRevision to HEAD** — Changed `argocd/app.yaml` `targetRevision` from `feature/prd-129-grafana-cloud-alloy-migration` back to `HEAD`
