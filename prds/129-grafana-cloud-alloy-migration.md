...
- [ ] **Milestone 3c: Ship logs through Alloy to Grafana Cloud Loki** — Add `loki.source.kubernetes` and `loki.write` components to Alloy River config to tail pod logs and forward to Grafana Cloud Loki. Use the existing `GRAFANA_CLOUD_API_TOKEN` for authentication. Validate using Grafana MCP `query_loki_logs` to find recent pod logs (e.g., `{namespace="dot-ai"}`)
